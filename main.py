...
def start_transcription():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Transcribe audio
        transcriptions = transcribe_audio(model_size, device, audio_path, include_timecodes, log)
...
transcription_textbox.grid(row=5, column=0, columnspan=3, sticky="ew", padx=5, pady=5)

ctk.CTkButton(frame, text="Embed Subtitles", command=start_embedding_thread).grid(row=6, column=0, columnspan=3, pady=10)

log_box = ctk.CTkTextbox(frame, height=100, width=600)
log_box.grid(row=7, column=0, columnspan=3, sticky="ew", padx=5, pady=5)
